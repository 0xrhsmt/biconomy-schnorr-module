...
import Schnorrkel, { Key } from "@borislav.itskov/schnorrkel.js/src/index";
import { wallet2 } from "@borislav.itskov/schnorrkel.js/tests/config.js";
import DefaultSigner from "@borislav.itskov/schnorrkel.js/utils/DefaultSigner";
import { _generatePk } from "@borislav.itskov/schnorrkel.js/src/core";
...
describe("Multi Sign Tests", function () {
  ...
  it("should fail if a signer tries to sign twice with the same nonce", async function () {
    ...
    const { signature: s, challenge: e } = signerOne.multiSignMessage(
      msg,
      publicKeys,
      publicNonces
    );
    expect(
      signerOne.multiSignMessage.bind(signerOne, msg, publicKeys, publicNonces)
